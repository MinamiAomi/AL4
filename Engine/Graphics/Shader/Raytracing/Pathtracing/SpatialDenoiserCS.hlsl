...
Texture2D<float32_t4> g_Position : register(t3, space1);
...
float32_t GeometryWeight(in float32_t3 delta) {
    float32_t dist2 = dot(delta, delta);
    return min(exp(-dist2 / 0.125f), 1.0f);
}
...
float32_t3 ATrousFilter(in int32_t2 center, in int32_t2 textureSize) {
    ...
    float32_t3 centerNormal = g_Normal[center].rgb * 2.0f - 1.0f;
    float32_t3 centerPosition = g_Position[center].r;
    ...
    for (int32_t y = 0; y < 5; ++y) {
        for (int32_t x = 0; x < 5; ++x) {
            ...
            float32_t3 normal = g_Normal[samplePixel].rgb * 2.0f - 1.0f;
            float32_t3 position = g_Position[samplePixel].r;

            if (metallicRoughness.x != centerMetallicRoughness.x || metallicRoughness.y != centerMetallicRoughness.y) { continue; }

            float32_t albedoWeight = GeometryWeight(albedo.rgb - centerAlbedo.rgb);
            float32_t normalWeight = GeometryWeight(normal - centerNormal);
            float32_t positionWeight = GeometryWeight(position - centerPosition);

            float32_t w = KernelWeight(int32_t2(x, y)); // * albedoWeight * normalWeight * positionWeight;
...
void main(uint32_t3 DTid : SV_DispatchThreadID, uint32_t3 GTid : SV_GroupThreadID) {
    ...
    // フィルター
    color.rgb = ATrousFilter(targetPixel, textureSize);
